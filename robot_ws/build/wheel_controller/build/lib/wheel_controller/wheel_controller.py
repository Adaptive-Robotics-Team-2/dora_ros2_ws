...
class WheelController(Node):
    ...
    def prepare_serial_data(self, v_fl, v_fr, v_rl, v_rr):
        def convert_to_bytes(velocity):
            velocity = int(velocity * 100)
            return velocity.to_bytes(4, byteorder='big', signed=True)
        
        buffer = bytearray(self.BUFFERSIZE)
        buffer[0] = b'\x10'  # Header
        buffer[1:5] = convert_to_bytes(v_fl)
        buffer[5:9] = convert_to_bytes(v_fr)
        buffer[9:13] = convert_to_bytes(v_rl)
        buffer[13:17] = convert_to_bytes(v_rr)
        buffer[-1] = sum(buffer[1:-1]) & 0xFF  # Checksum
    ...
    def send_to_serial(self, data):
        try:
            if self.serial_port.in_waiting > 0:
                ...
                if header != b'\x10':
                    self.get_logger().error(f"Invalid header: {header}")
                    return
